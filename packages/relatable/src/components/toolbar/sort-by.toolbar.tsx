...
export default function SortByToolbar() {
  ...
  return <ToolbarPopup
    ...
    content={<SortByPopup columns={columns} sortBy={sortBy} onCustomSortChange={onCustomSortChange}/>}
    selectedToolbar={selectedToolbar}
    onClose={clearToolbar}>
    <Menu.Item name="sort" onClick={() => setToolbar(withSorting.name)}>
      <Icon name='sort'/>
      {arrayHasItems(sortBy) && <Label>{sortBy.length}</Label>}
    </Menu.Item>
  </ToolbarPopup>;
}
...
function SortByPopup({ columns, sortBy, onCustomSortChange }: any) {
  ...
  return <div className="relatable__toolbar-sort-by-popup">
    ...
    {showForm
      ? <SortByForm columns={columns} onCustomSortChange={onCustomSortChange} onClose={() => setShowForm(false)}/>
